...
guidesRouter.openapi({
  ...
  path: '/',
  security: [{ Bearer: [] }],
  responses: { 200: { content: { 'application/json': { schema: z.array(GuideSchema) } } } }
}, async (c) => {
  const utilisateurId = c.req.query('utilisateurId');
  if (utilisateurId) {
    return c.json(await guidesService.findByUtilisateurId(utilisateurId), 200);
  }
...
guidesRouter.openapi({
  ...
  security: [{ Bearer: [] }],
  responses: { 200: { content: { 'application/json': { schema: GuideSchema } } } }
}, async (c) => {
...
guidesRouter.openapi({
  ...
  path: '/',
  security: [{ Bearer: [] }],
  request: { body: { content: { 'application/json': { schema: GuideSchema.omit({ id: true }) } } } },
  responses: { 201: { content: { 'application/json': { schema: GuideSchema } } } }
}, async (c) => {
...
guidesRouter.openapi({
  ...
  request: { 
    ...
    body: { content: { 'application/json': { schema: GuideSchema.partial() } } }
  },
  security: [{ Bearer: [] }],
  responses: { 200: { content: { 'application/json': { schema: GuideSchema } } } }
}, async (c) => {
